...
- **Unplayable / Burn / Find** (Writer) — **Unplayable** cards can't be played from hand (`CardInstance.unplayable`, printed or granted by Trash Can); **Burn** spends them from the hand, playing each burned card's effects for free off the `CardsBurned` event (`src/cards/match/burn.ts`, `docs/triggers.md`) — and is a *cost*: a Burn card needs that many Unplayable cards in hand. **Find** draws like Draw but records Unplayable hits in `Combatant.unplayablesFound`, which the card's "If you find an unplayable card, …" rider reads (`IfFoundUnplayable`).
