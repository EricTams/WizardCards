...
- **Combatant** — the player or an enemy: id, hp/maxHp, the resources `block`, `shield`, `energy`, `poison`, `power`, `bravery`, plus `clouds[]`, `minions[]`, and `persistents[]`. Build one with `makeCombatant()`. (`src/engine/state`)
...
- **Claw** (Crab) — a card keyword: when the card is discarded, it plays for free. (Not modeled yet.)
...
- **Persistent** — an ongoing card that stays in play and keeps applying its effect (all five characters have some). Modeled as `Combatant.persistents[]` + `PersistentDef` trigger behavior; a first set (Winter, Autumn, Spring, Summer, Static, Rot Away, Consuming) is live. See `docs/triggers.md`.
