...
- ✅ **The Crab** (`src/cards/definitions/crab.ts`): 20 cards + Crab Trap, playable from character select on the `beach` theme, with the **Claw** keyword (`src/cards/match/claw.ts`), `discard N cards`, scaling off `cardsDiscardedThisTurn`, and the Seashell relic (opening hand of 6 instead of 5).
