...
- ✅ **Card choices** for discards and burns: the player picks which cards (mulligan-style pause — `PendingChoice` + `resolvePendingChoice`, see `docs/battle.md` §4b); the AI keeps the deterministic defaults. Still to extend to: cloud removal ("your choice" per the design), minion discards, and discard-pile retrieval (Dry Out).
