...
4. **Turns.** Each turn you start with base energy (1) plus Lightning-cloud energy, draw 1, and play cards while you can afford them. Cards cost 1 by default; energy cards let you play more.
   - **Running out of cards refills your hand:** the moment a combatant's hand hits zero mid-battle, they draw `HAND_REFILL` (3; Brain in a Jar: 4 via `bonusRefillDraw`) new cards — playing your last card counts. The refill settles at **step boundaries** (`settleHandRefills` — after a card fully resolves, after end-of-turn discards, at the end of the start-of-turn cascade), never mid-card: a mid-card refill could reshuffle the played card out of the discard before its own pile-movement resolves, and would double-fill hands for empty-then-redraw cards (Refresh, Exoskeleton). It's symmetric (enemies refill too), and inert during setup/mulligan or when no cards remain anywhere.
4b. **Card choices.** When a *human's* card discards or burns with a genuine choice (more eligible cards than the effect takes), the resolution **pauses**: `runOrPause` raises `SetPendingChoice`, `GameState.pending` holds the not-yet-applied remainder (`queued`, plain actions — so a paused game still serializes/replays), and the battle screen turns hand clicks into picks. `resolvePendingChoice(state, uids)` validates the picks (in-hand; Unplayable-only for burns; exactly `count` of them), applies the concrete `DiscardCards`/`BurnCards` *with* `uids`, and resumes the queue — which may pause again on the next choice. The pause sits mid-card deliberately: Double Draw ("Draw 2. Discard 2.") must offer the post-draw hand. Three boundaries never pause: the AI (always the deterministic default — rightmost discard, leftmost burn), whole-hand/whole-pool effects (no real choice), and discards fired *inside* the trigger cascade (a Molt free play's own discard auto-resolves — the cascade stays atomic). The end-of-turn Fog discard is interactive too; its pause suspends `EndTurn` itself, so the enemy turn starts only after the pick.
